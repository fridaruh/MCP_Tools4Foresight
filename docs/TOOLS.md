# Referencia de tools — `mcp-tools4foresight`

> La superficie MCP completa: 18 tools, 7 resources y 6 prompts sobre el acervo de
> foresight de tools4foresight.com. Este documento describe **lo que expone el
> servidor MCP**, no la API HTTP subyacente — para eso está `docs/API.md`, que es el
> contrato entre `MCP_Tools4Foresight` y `x-likes-curator`. Para el vocabulario del
...
Lista las señales publicadas con filtros combinables (categoría, PESTEL, horizonte,
...
{
  "data": [
    { "id": "c7f3a914-20bc-4d4c-9c51-940f372e0d8a", "vitality": 0.87, "category": "Gobernanza y regulación", "...": "..." }
  ],
...
> Búsqueda de texto libre sobre el título, el texto original, el TL;DR y el 'por qué importa' de las señales publicadas. Úsala cuando busques un término concreto. Para explorar por CERCANÍA CONCEPTUAL (temas parecidos aunque no compartan palabras), parte de un resultado y usa `get_signal_neighbors`.
...
```json
{
  "data": [{ "id": "c7f3a914-20bc-4d4c-9c51-940f372e0d8a", "title": "El Parlamento Europeo aprueba el reglamento de agentes autónomos" }],
...
> Ficha completa de una señal publicada: TL;DR, por qué importa, impacto en el desarrollo de la IA y en la interacción entre humanos, categoría, dimensiones PESTEL, vitalidad y el tema al que pertenece. La fecha `likedAt` es estimada (muéstrala con `~`); `tweetCreatedAt` sí es exacta.
...
> Las seis dimensiones PESTEL (Political, Economic, Social, Technological, Environmental, Legal) con su conteo de señales. OJO: cada señal lleva como máximo 2 dimensiones, así que la suma de los conteos puede superar el total de señales publicadas — eso es esperado, no una inconsistencia.
...
Conteos del acervo, rango de fechas, última corrida y las constantes reales del
modelo.
...
- Vida media de señales huérfanas: 30 días
...
```json
{
  "data": {
    ...
    "domain": { "halfLifeDays": 30, "orphanHalfLifeDays": 30, "deadThreshold": 1, "linkThreshold": 0.55, "minThemeSize": 3, "maxMacroPerHorizon": 5 }
  }
}
```
...
|---|---|---|---|---|
| `foresight-overview` | `foresight://overview` | No | — | Conteos, rango de fechas y constantes del modelo (`/meta`). La puerta de entrada al acervo. |
...
no estables, el grafo cubre solo señales publicadas, y consultar
...
| `403` | `tools4foresight rechazó el acceso (permisos insuficientes de la API key). No reintentes: no es un problema pasajero.` |
...
- **`~` en toda fecha `likedAt`** (y en cualquier fecha derivada de ella, como
  ...
  `tweetCreatedAt` y `publishedAt`, en cambio, sí son exactas y se muestran **sin**
