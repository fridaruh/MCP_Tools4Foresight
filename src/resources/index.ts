 ...
 * el sobre de paginación, no `MetaDTO`, el resumen del acervo) — se renderiza
...
function renderOverview(meta: MetaDTO): string {
  ...
  return [
    "# Resumen del acervo — tools4foresight",
    "",
    `- **Señales publicadas**: ${counts.publishedSignals}`,
...
export function registerResources(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerResource(
    ...
    }),
    {
      ...
      description: "TL;DR, por qué importa, impacto, categoría, PESTEL, vitalidad y tema de una señal publicada.",
      mimeType: "text/markdown",
    },
    async (uri, variables) =>
      guardedRead(uri, async () => {
