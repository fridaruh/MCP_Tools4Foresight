...
export function formatSignalDetail(signal: SignalDetailDTO): string {
  ...
  // `publishedAt` es exacta (cuándo Frida la publicó), no una estimación de
  // like: sin virgulilla, igual que `tweetCreatedAt`.
  if (signal.publishedAt) lines.push(`- **Publicada**: ${formatDate(signal.publishedAt)}`);
