...
export class T4FApiError extends Error {
  ...
  messageForModel(): string {
    switch (this.status) {
      case 401:
        return 'La API key de tools4foresight es inválida o falta. Revisa T4F_API_KEY en la configuración del servidor MCP. No reintentes.';
      case 403:
        return 'tools4foresight rechazó el acceso (permisos insuficientes de la API key). No reintentes: no es un problema pasajero.';
      case 404:
        return 'No existe ese id entre el contenido publicado. Usa list_signals o list_themes para obtener ids válidos.';
      case 429:
